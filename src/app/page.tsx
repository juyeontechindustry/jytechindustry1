    ...
    <>
      ...
      <section id="about" style={{padding:"48px 16px"}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          <ul style={{listStyle:"none",padding:0,display:"grid",gap:16,gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))"}}>
            <li style={card}><h3>안전</h3><p>표준 작업 절차·정기 교육·보호구 상시 비치</p></li>
            <li style={card}><h3>품질</h3><p>검측 체크리스트·자재 이력·철저한 마감</p></li>
            <li style={card}><h3>납기</h3><p>공정 관리·리스크 선대응·신속 의사결정</p></li>
          </ul>
        </div>
      </section>
      ...
      <section id="services" style={{padding:"48px 16px",background:"#fafafa"}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          <h2>사업분야</h2>
        <ul style={{listStyle:"none",padding:0,display:"grid",gap:16,gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))"}}>
            <li style={card}><h3>건축/리모델링</h3><p>상가·주거 리모델링, 내/외장 공사</p></li>
            <li style={card}><h3>토목/외부</h3><p>배수·포장, 옹벽, 조경·외부 시설물</p></li>
            <li style={card}><h3>설비</h3><p>배관/배수, 환기, 위생 설비</p></li>
            <li style={card}><h3>전기</h3><p>조명, 배선, 분전반, 전력 증설</p></li>
          </ul>
        </div>
      </section>
        ...
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          <ul style={{listStyle:"none",padding:0,display:"grid",gap:12,gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))"}}>
            {Array.from({length:6}).map((_,i)=>(
              <li key={i} style={{height:120,border:"1px dashed #e5e7eb",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",color:"#667"}}>사례 {i+1}</li>
            ))}
          </ul>
        </div>
      </section>
